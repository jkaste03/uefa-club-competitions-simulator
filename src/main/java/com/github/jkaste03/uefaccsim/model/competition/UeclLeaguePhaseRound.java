...
public class UeclLeaguePhaseRound extends LeaguePhaseRound {
    ...
    protected void draw() {
        ...
        List<NonKnockoutTie> resultTies = new ArrayList<>();
        for (int ei = 0; ei < allEdges.size(); ++ei) {
            ...
            int x = sol[ei];
            if (x == 0)
                resultTies.add(new NonKnockoutTie(idxToClub[u], idxToClub[v]));
            else
                resultTies.add(new NonKnockoutTie(idxToClub[v], idxToClub[u]));
        }
